...
public class DFA {
    ...
    try {
      ...
      while (true) {
        ...
        if (token >= 0) {
          ...
          if (db)
            pr(" found new final state: " + token);
  ...
  private void read(InputStream s) throws IOException {
    ...
    for (int i = 0; i < tokenNameCount; i++) {
      ...
      if (false) {
        warning("dumping debug");
        pr("added " + n + ": " + (i + Token.T_USER));
      }
    }
