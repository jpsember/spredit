...
public final class MyMath {
  ...
  public static float ceilf(float n) {
    return (float) Math.ceil(n);
  }

  public static float floorf(float n) {
  ...
  public static Point randomPointInDisc(Random random, Point origin,
      float radius) {
    float val = radius * sqrtf(random.nextFloat());
  ...
  public static Point midPoint(Point  s1,Point s2) {
    return interpolateBetween(s1, s2, .5f);
  }
}
