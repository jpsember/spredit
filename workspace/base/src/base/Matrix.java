...
public class Matrix {
  ...
  public static Matrix mult(Matrix ma, Matrix mb, Matrix md) {
    ...
    float[] a = ma.coeff, b = mb.coeff;

    float[] d = md.coeff;
    if (md == ma || md == mb)
      d = new float[d.length];
   ...
   */
  public Matrix(float[] array, int offset) {
    int t = offset;
   ...
   */
  public Matrix(int height, int width, float[] coeff) {
   ...
   */
  public Matrix(int height, int width, float[] coeff, int offset) {
    this(height, width);
   ...
   * @param offset index of first coefficient within array
   */
  private void readValues(float[] dc, int offset) {
    for (int i = 0; i < coeff.length; i++)
      coeff[i] = dc[i + offset];
  }
   ...
   * @return result
   */
  public FlPoint2 apply(FlPoint2 pt, FlPoint2 dest) {
   ...
   * @return result
   */
  public FlPoint2 apply(IPoint pt) {
    return apply(pt.x, pt.y, null);
  }
   ...
   * @return result
   */
  public FlPoint2 apply(float x, float y, FlPoint2 dest) {
   ...
   * @return destination matrix
   */
  public Matrix scale(float sx, float sy, Matrix dest) {
   ...
   * @param y
   */
  public Matrix translate(float x, float y, Matrix dest) {

    float[] c;
    if (dest == null) {
      dest = new Matrix(SIZE2D);
    }
    c = dest.coeff;
    if (dest == this) {
      c = new float[SIZE2D * SIZE2D];
    }

    float[] v = coeff();

    c[0 * SIZE2D + 0] = v[0 * SIZE2D + 0];
    c[0 * SIZE2D + 1] = v[0 * SIZE2D + 1];
    c[0 * SIZE2D + 2] = v[0 * SIZE2D + 0] * x + v[0 * SIZE2D + 1] * y
        + v[0 * SIZE2D + 2];

    c[1 * SIZE2D + 0] = v[1 * SIZE2D + 0];
    c[1 * SIZE2D + 1] = v[1 * SIZE2D + 1];
    c[1 * SIZE2D + 2] = v[1 * SIZE2D + 0] * x + v[1 * SIZE2D + 1] * y
        + v[1 * SIZE2D + 2];
    c[2 * SIZE2D + 0] = v[2 * SIZE2D + 0];
    c[2 * SIZE2D + 1] = v[2 * SIZE2D + 1];
    c[2 * SIZE2D + 2] = v[2 * SIZE2D + 0] * x + v[2 * SIZE2D + 1] * y
        + v[2 * SIZE2D + 2];
   ...
   */
  public Matrix translate(FlPoint2 tr, Matrix dest) {
    return translate(tr.x, tr.y, dest);
  }
   ...
   */
  public static Matrix getRotate(float ang) {
   ...
   * @return 2d matrix
   */
  public static Matrix getTranslate(FlPoint2 pt, boolean neg) {return getTranslate(pt.x,pt.y,neg);}
//  
...
//  }
   ...
   * @return 2d matrix
   */
  public static Matrix getTranslate(float x, float y, boolean neg) {
    Matrix m = new Matrix(3);
    m.setIdentity();
    float[] v = m.coeff();
    float sign = neg ? -1 : 1;
    v[SIZE2D * 0 + SIZE2D - 1] = x * sign;
    v[SIZE2D * 1 + SIZE2D - 1] = y * sign;
    return m;
  }
   ...
   * @return 2d matrix
   */
  public static Matrix getScale(FlPoint2 scale) {
   ...
   * @return 2d matrix
   */
  public static Matrix getScale(float sx, float sy) {
    Matrix m = new Matrix(3);
    m.setIdentity();
    float[] v = m.coeff();
    v[SIZE2D * 0 + 0] = sx;
   ...
   * @return destination matrix
   */
  public Matrix scale(float s, Matrix dest) {
  ...
  public Matrix transpose(Matrix dest) {
    ...
    float[] ret = new float[coeff.length];
  ...
  //   */
  //  public float[] solve(float[] b, float[] result) {
  //
   ...
   */
  public static String toString(Matrix M, float[] B) {
  ...
  public static String toString(Matrix M, float[] B, int bCols) {
